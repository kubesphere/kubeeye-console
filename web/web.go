...
// RunWebService run an WebServer instance using given options
func RunWebService(s *ServerRunOptions) error {
	...
	if s.TlsCertFile != "" && s.TlsPrivateKey != "" {
		klog.Infof("Start listening on %d", s.SecurePort)
		err = http.ListenAndServeTLS(fmt.Sprintf(":%d", s.SecurePort), s.TlsCertFile, s.TlsPrivateKey, mux)
	} else {
